<?php
	...
	class functions{
		...
		//new function
		public function teacher_subject($key){
		...
		public function quiz_answer_details($id, $secId, $qid){
			...
			$query = $this->dbase->query("SELECT * FROM portal_quizzes WHERE sectionid = \"$secId\" AND subjectid = \"$id\" AND quizzId = \"$qid\" AND quiztype = \"respondent\" AND stat != 0");
			return $query;
		...
		public function all_schoolmate_user(){

			$query = $this->dbase->query("SELECT * FROM portal_students a, portal_user_details b, portal_users c WHERE a.studentId = b.userId AND b.userId = c.userId ORDER BY a.studentId ASC");
			return $query;

		}

		public function search_schoolmate_user($id){

			$query = $this->dbase->query("SELECT * FROM portal_students a, portal_user_details b, portal_users c WHERE
										  a.studentId = b.userId AND b.userId = c.userId AND b.firstname LIKE \"%$id%\" OR
										  a.studentId = b.userId AND b.userId = c.userId AND b.middlename LIKE \"%$id%\" OR
										  a.studentId = b.userId AND b.userId = c.userId AND b.lastname LIKE \"%$id%\"
		...
		public function perstudInfo($key, $syear){
			...
			$query = $this->dbase->query("SELECT * FROM tblschool_studinfo a, tblschool_registration b WHERE a.accid = b.accid AND a.accid = \"$key\" AND b.sy_ayid = \"20\"");
			return $query;
		...
		public function profileQuizzes($id){
			...
			$query = $this->dbase->query("SELECT * FROM portal_quizzes WHERE uploadedby = \"$id\" AND stat != 0");
			return $query;
